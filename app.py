...
def view_logins():
    try:
        ...
        for doc in docs:
            ...
            # Convert the Firestore timestamp to Syria timezone and a readable format (optional)
            if 'timestamp' in login_data and login_data['timestamp'] is not None:
                # Convert Firestore timestamp to Python datetime
                utc_time = login_data['timestamp'].replace(tzinfo=pytz.UTC)
                syria_time = utc_time.astimezone(syria_tz)
                login_data['timestamp'] = syria_time.strftime("%Y-%m-%d %H:%M:%S")
